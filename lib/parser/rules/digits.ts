    ...
    parseRule(rule: string): Operands {
        if (rule.startsWith("digits:")) {
            ...
            if (operands.length !== 1) {
                throw new Error("Invalid rule: \"digits\" requires 1 operand eg. digits:4");
            }

            const value = Number(operands[0]);

            if (Number.isNaN(value) || !Number.isInteger(value)) {
                throw new Error("Invalid rule: \"digits\" requires that the operand to be an integer eg. digits:4");
            }
